...
namespace KxLib.Utilities {
    /// <summary>
    /// 字符串操作类
    /// GetStrArray(string str, char speater, bool toLower)  把字符串按照分隔符转换成 List
    /// GetStrArray(string str) 把字符串转 按照, 分割 换为数据
    /// GetArrayStr(List list, string speater) 把 List 按照分隔符组装成 string
    /// GetArrayStr(List list)  得到数组列表以逗号分隔的字符串
    /// GetArrayValueStr(Dictionary<int, int> list)得到数组列表以逗号分隔的字符串
    /// DelLastComma(string str)删除最后结尾的一个逗号
    /// DelLastChar(string str, string strchar)删除最后结尾的指定字符后的字符
    /// ToSBC(string input)转全角的函数(SBC case)
    /// ToDBC(string input)转半角的函数(SBC case)
    /// GetSubStringList(string o_str, char sepeater)把字符串按照指定分隔符装成 List 去除重复
    /// GetCleanStyle(string StrList, string SplitString)将字符串样式转换为纯字符串
    /// GetNewStyle(string StrList, string NewStyle, string SplitString, out string Error)将字符串转换为新样式
    /// SqlSafeString(string String, bool IsDel)
    /// </summary>
